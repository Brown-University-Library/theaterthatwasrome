...
class MissingGenreError(ValueError):
    """
    Identifies an annotation genre absent from the local genre list.
    """

    def __init__(self, genre_text: str) -> None:
        """
        Keeps the unmatched genre available for the correction message.
        Called by: Annotation.get_form_data()
        """
        self.genre_text = genre_text
        super().__init__('The annotation genre is missing from the local genre list.')
...
class Biography(models.Model):
    id: int
    essay_set: 'models.Manager[Essay]'

    name = models.CharField(max_length=254, help_text='Enter name as it appears in the book metadata')
    trp_id = models.CharField(
        max_length=15,
        unique=True,
        blank=True,
        help_text='Optional: This value will be auto-generated by the server if the field is left blank or non-unique value is entered',
    )
    ...
    external_id = models.CharField(
        ...
        null=True,
        blank=True,
        help_text='Optional: enter Ulan id in the form of a URL; if there is no Ulan id, enter LCCN in the form of a URL',
    )
    birth_date = models.CharField(
        max_length=25,
        null=True,
        blank=True,
        help_text='Optional: enter birth date as yyyy-mm-dd (for sorting and filtering)',
    )
...
class Essay(models.Model):
    ...
    title = models.CharField(max_length=254)
    text = models.TextField()
    pids = models.CharField(
        max_length=254,
        null=True,
        blank=True,
        help_text='Comma-separated list of pids for books or prints associated with this essay.',
    )
    people = models.ManyToManyField(Biography, blank=True, help_text='List of people associated with this essay.')
...
class Shop(models.Model):
    ...
    pids = models.CharField(
        ...
        null=True,
        blank=True,
        help_text='Comma-separated list of pids for books or prints associated with this shop.',
    )
...
class Page(BDRObject):
    ...
    def url(self):
        if self.parent is None:
            raise ValueError('A book page requires a parent book to build its URL.')
...
class Print(Page):
    ...
    def get_print_info_from_solr_doc(solr_doc, collection):
        ...
        for search_term, attribute in [('chinea', 'in_chinea'), ('buonanno', 'in_buonanno')]:
            if collection == search_term or (
                re.search(search_term, title, re.IGNORECASE)
                or (re.search(search_term, solr_doc['subtitle'][0], re.IGNORECASE) if 'subtitle' in solr_doc else False)
            ):
...
def annotations_by_books_and_prints(bio_name, group_amount=50):
    ...
    while i < num_pages:
        ...
        for page in looked_up_pages:
            try:
                book_id = page['rel_is_part_of_ssim'][0].split(':')[-1]
...
class Annotation:
    ...
    def from_form_data(cls, image_pid, annotator, form_data, person_formset_data, inscription_formset_data, pid=None):
        return cls(
            ...
            pid=pid,
            form_data=form_data,
            person_formset_data=person_formset_data,
            inscription_formset_data=inscription_formset_data,
        )

    @classmethod
    def from_pid(cls, pid):
        r = bdr_client.request('GET', annotation_xml_url(pid))
    ...
    def get_form_data(self) -> dict:
        """
        Reads form values after resolving the annotation's existing genre.
        Called by: annotation_forms.get_bound_edit_forms()
        """
        if not self._form_data:
            ...
            if title1_lang:
                form_data['title_language'] = title1_lang
            if len(self._mods_obj.title_info_list) > 1:
                form_data['english_title'] = self._mods_obj.title_info_list[1].title
            if self._mods_obj.genres:
                genre_text = self._mods_obj.genres[0].text
                if genre_text:
                    try:
                        genre = Genre.objects.get(text=genre_text)
                    except Genre.DoesNotExist as exc:
                        raise MissingGenreError(genre_text) from exc
    ...
    def get_mods_obj(self, update=False):
        ...
        if self._form_data['abstract']:
            ...
            if self._mods_obj.abstract is None:
                raise ValueError('Could not create the annotation abstract.')
        ...
        if self._form_data['impression_date']:
            ...
            if self._mods_obj.origin_info is None:
                raise ValueError('Could not create the annotation origin information.')
    ...
    def _get_params(self):
        ...
        params['rels'] = json.dumps(
            {'isMemberOfCollection': settings.TTWR_COLLECTION_PID, 'isAnnotationOf': self._image_pid}
        )
        params['rights'] = json.dumps(
            {'parameters': {'owner_id': app_settings.BDR_IDENTITY, 'additional_rights': 'BDR_PUBLIC#display'}}
        )
    ...
    def save_to_bdr(self):
        ...
        r = bdr_client.request('POST', app_settings.BDR_POST_URL, data=params)
        return {'pid': r.json()['pid']}

    def update_in_bdr(self):
        params = self._get_update_params()
        bdr_client.request('PUT', app_settings.BDR_POST_URL, data=params)
